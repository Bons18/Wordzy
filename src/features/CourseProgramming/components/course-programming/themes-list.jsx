...
export default function ThemesList({
  ...
  const getAvailableThemeOptions = (currentThemeId) => {
    ...
    const currentSelection =
      currentTheme?.selectedTheme?.value || currentTheme?.selectedTheme;
    ...
    setLevels(
      levels.map((level) => {
        if (level.id === levelId) {
          const updatedThemes = level.themes.map((theme) =>
            theme.id === themeId
              ? { ...theme, selectedTheme: selectedOption }
              : theme
          );
          return { ...level, themes: updatedThemes };
        }
  ...
  const deleteTheme = (levelId, themeId) => {
    setLevels(
      levels.map((level) => {
        if (level.id === levelId) {
          const updatedThemes = level.themes.filter(
    ...
    setLevels(
      levels.map((level) => {
        if (level.id === levelId) {
          const updatedThemes = level.themes.map((theme) =>
            theme.id === themeId
              ? { ...theme, showActivities: !theme.showActivities }
              : theme
          );
          return { ...level, themes: updatedThemes };
        }
  ...
  const getThemeDisplayName = (theme) => {
    if (theme.selectedTheme) {
      ...
      if (
        typeof theme.selectedTheme === "object" &&
      ...
      // Si selectedTheme es un string (ID), buscar en createdTopics
        ...
        const foundTopic = createdTopics.find(
          (topic) => topic.value === theme.selectedTheme
        );
      ...
      // Si selectedTheme tiene value, buscar por value
        ...
        const foundTopic = createdTopics.find(
          (topic) => topic.value === theme.selectedTheme.value
        );
        return foundTopic ? foundTopic.label : "Tema no encontrado";
      }
    }
  ...
  const calculateTotalThemeValue = () => {
    return (
      level.themes?.reduce(
        (total, theme) => total + (theme.progress || 0),
        0
      ) || 0
    );
  };
        ...
        <div className="flex justify-between items-center px-2 py-1 bg-gray-50 rounded-[10px]">
          ...
          <span
            className={`text-sm font-medium ${
              isValueValid ? "text-green-600" : "text-red-600"
            }`}
          >
            <span className="inline-flex items-center gap-1">
              {totalThemeValue}%{" "}
              {!isValueValid && <CircleAlert className="h-5 w-7" />}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                  <div>
                    ...
                    <CustomSelect
                      ...
                      onChange={(option) =>
                        updateTheme(level.id, theme.id, option)
                      }
                    />
                ...
                {theme.showActivities && (
                  <ActivitiesSection
                    ...
                    setLocalActiveTab={(tab) =>
                      setLocalActiveTab(level.id, theme.id, tab)
                    }
                    levels={levels}
                    setLevels={setLevels}
                  />
